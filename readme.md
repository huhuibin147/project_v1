...
玩家已有攻击/防御/速度/HP等属性和武器装备，但缺少核心战斗玩法。

- 地图上放置怪物（森林地图优先）
- 回合制战斗系统（玩家选择攻击/防御/使用物品/逃跑）
- 战斗胜利获得经验和掉落物品
- 与现有属性系统（攻击、防御、HP）直接联动
- 怪物配置文件（名称、属性、掉落表）
- 战斗UI（血条、技能按钮、战斗日志）

### Phase 3：任务系统（优先级：高）

意图识别已支持 `quest` 类型，NPC背景也提到了任务，但任务没有实际执行机制。

- NPC下发任务（收集物品、击杀怪物、传递消息）
- 任务进度追踪（UI面板显示当前任务）
- 任务完成奖励（经验、金币、好感度提升）
- 任务类型：主线任务、支线任务、日常任务
- 任务状态：未接取、进行中、已完成、已提交

### Phase 4：装备系统（优先级：高） ✅ 已完成

- ✅ 装备栏UI（武器、盾牌、头部、身体、饰品5个槽位）
- ✅ 装备影响角色属性（攻击+、防御+、速度+、HP+）
- ✅ 装备替换（新装备自动替换同槽位旧装备）
- ✅ 背包中装备按钮和属性加成显示
- ✅ 商店中装备属性加成显示
- 装备与战斗系统联动（待Phase 2战斗系统实现后联动）
- 装备耐久度（可选）
- 装备强化/升级（可选）

### Phase 5：等级成长系统（优先级：中）
