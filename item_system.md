...
玩家拥有**唯一**的全局背包和金币，保存在 `data/player_save.json` 中，包含：
- `inventory`：物品列表 `[{item_id, quantity}]`
- `gold`：金币数量

无论与哪个 NPC 交易，都使用同一个钱包和背包。NPC 只保存自己的商店库存和对话历史。
...
系统现在能更准确地识别交易意图：
- **购买识别**：玩家明确说出"买"、"要"、"给我"等关键词
- **出售识别**：玩家明确说出"卖"、"出售"等关键词
- **非交易识别**：询问价格、查看商品等不会误触发交易
...
| 给我来3个生命药水 | 好嘞！3个药水，收你150金币！ | 购买成功 |
| 我要卖给你一个草药 | 草药不错，15金币收了！ | 出售成功 |
| 有没有好一点的剑 | 精钢剑 200 金币，绝对值！ | 展示商品 |
| 铁剑多少钱？ | 铁剑80金币一把 | 不触发交易 |
| 你这里有什么？ | 我这里有铁剑、精钢剑... | 不触发交易 |
| 这个太贵了便宜点 | 好吧好吧，看在你是老顾客的份上... | 好感度影响 |

### 4.2 商店面板交易

玩家靠近 NPC 后按 **E**，弹出交互选项面板，选择「商店」即可打开该 NPC 的商店。

**流程**：
1. 靠近 NPC 并按 E，弹出「对话 / 商店」选项
2. 选择「商店」打开商店面板
3. 点击「购买」或「出售」按钮直接交易
4. 交易结果实时显示
...
NPC 数据保存在 `data/{npc_id}_save.json`（含商店库存、收购仓库和对话历史）：
