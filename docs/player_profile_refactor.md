...
| 优化项 | 状态 | 说明 |
|--------|------|------|
| 存档备份与恢复 | ✅ 完成 | 保存前自动备份 `.json.bak`，加载失败时从备份恢复，`_verify_save()` 完整性校验 |
| 属性计算缓存 | ✅ 完成 | `StatsCache` 类，MD5 哈希判断是否需要重算，装备/天赋/等级不变时复用结果 |
| 物品索引优化 | ✅ 完成 | `_inventory_index` 字典，查询复杂度 O(N)→O(1) |
| 属性计算模块化 | ✅ 完成 | `_calculate_stats()` + `_apply_stats()` 两阶段分离 |
| 属性上限校验 | ✅ 完成 | 所有属性限制 [1, 99999] |
| 数据序列化拆分 | ✅ 完成 | `_to_dict()` + `_from_dict()` 职责分离 |

**新增类/方法**：
- `StatsCache` — 属性计算缓存
- `_backup_save()` / `_restore_from_backup()` — 存档备份恢复
- `_verify_save()` — 存档校验
- `_rebuild_inventory_index()` — 物品索引重建
