"""Event-driven system for affix and talent triggers with multi-enemy support."""
            ...
            if eff.get("action") == "apply_effect_to_monster":
                ...
                if random.random() < chance:
                    for m in session.alive_monsters():
                        ...
                        m.effects = add_effect(m.effects, new_eff, monster_config=m.config)
    ...
    def on_attack_handler(**kwargs):
        ...
        for eff in passives.get("on_attack", []):
            if eff.get("action") == "lifesteal":
                ...
                if random.random() < chance and target:
                    new_eff = StatusEffect("bleed", eff.get("duration", 3),
                                           source="talent", stack=1)
                    target.effects = add_effect(target.effects, new_eff, monster_config=target.config)
            ...
            for eff in affix.get("effects", []):
                if eff.get("type") == "reflect":
                    ...
                    if random.random() < trigger_chance:
                        ...
                        target = session.get_target()
                        if target:
                            target.hp = max(0, target.hp - reflect_dmg)
